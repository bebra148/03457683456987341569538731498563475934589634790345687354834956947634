...
function ultimate.Aim(cmd)
    ...
    if ultimate.cfg.vars["LerpTime"] then    
        local lerpTime = 0.1 -- фиксированное значение, если LerpTime = true
        if serverArriveTick - targetTime < 0.2 then
            ded.NetSetConVar("cl_interpolate", "0")
            ded.NetSetConVar("cl_interp", "0")
            ded.SetCommandTick(cmd, ultimate.TIME_TO_TICKS(targetTime + lerpTime))
        else
            ded.NetSetConVar("cl_interpolate", "0")
            ded.NetSetConVar("cl_interp", "0")
            ded.SetCommandTick(cmd, ultimate.TIME_TO_TICKS(targetTime + lerpTime))
    ...
    if ultimate.cfg.vars["LagCompensation"] and ultimate.target ~= nil then
        ...
        -- Если GetLerpTime() существует:
        local lerpTime = ultimate.GetLerpTime and ultimate.GetLerpTime() or 0.1
        ultimate.target:SetNWFloat("LerpTimeValue", lerpTime)
    end
